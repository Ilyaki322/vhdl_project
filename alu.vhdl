...
architecture ALU_Logic of ALU is
    ...
    signal sign_delay : std_logic_vector(1 downto 0) := (others => '0');
    begin
        process(clk)
        begin
                if reset = '0' then
                    ...
                    if rising_edge(clk) then
                        sign_delay(1) <= result(result'length - 1);
                    end if;

                    if enable = '0' then
                        zero_delay(0) <= zero_delay(1);
                        sign_delay(0) <= sign_delay(1);
                    end if;
                    zero_flag <= zero_delay(0) or zero_delay(1);
                    sign_flag <= sign_delay(0) or sign_delay(1);
                end if;
        end process;
